extension ValueReducers {
    ...
    public struct Fetch<Value>: ValueReducer {
        public struct _Fetcher: _ValueReducerFetcher {
            let _fetch: @Sendable (Database) throws -> transferring Value
            
            public func fetch(_ db: Database) throws -> transferring Value {
                assert(db.isInsideTransaction, "Fetching in a non-isolated way is illegal")
                return try _fetch(db)
            }
        }
        
        private let _fetch: @Sendable (Database) throws -> transferring Value
        
        /// Creates a reducer which passes raw fetched values through.
        init(fetch: @escaping @Sendable (Database) throws -> transferring Value) {
            self._fetch = fetch
        }
